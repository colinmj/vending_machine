  ...
  buyProduct(product, amount) {
    ...
    if (this.products[product] && this.products[product].price === amount) {
      ...
      if (change > 2) {
        ...
        if (change >= 1 && this.coins.loonie.count > 0) {
          let loonies = change / 1;
          loonies = Math.floor(loonies);
          let quarter = change - loonies;
          let quarters = quarter / 0.25;
          if (change % 1 === 0) {
            this.coins.loonie.amount--;
            return `${loonies} loonie`;
          } else {
            let quarterAmount;
            if (quarters === 1) {
            ...
            this.coins.loonie.amount--;
            this.coins.quarter.amount = this.coins.quarter.amount - quarters;
            return `${loonies} loonie ${quarters} ${quarterAmount}`;
          }
        }
